...
exports.getAllBooks = async (req, res) => {
  try {
    if (!req.session.user) {
      return res.send(
        'First Enter your login credential to access to all books'
      )
    }
...
exports.getAddBook = async (req, res) => {
  try {
    if (!req.session.user) {
      return res.send(
        'First Enter your login credential to access to adding books'
      )
    }
    if (req.session.user.isAdmin == false) {
  ...
  try {
    ...
    async function createUserCollection(userid) {
      ...
      try {
        ...
        console.log(
          `User collection "${userCollectionName}" created successfully`
        )
      } catch (err) {
...
exports.addBook = async (req, res) => {
  try {
    const { title, author, Type, price, quantity, image } = req.body
...
exports.addToCart = async (req, res) => {
  try {
    ...
    const userCollectionName = generateUserCollectionName(
...
exports.updateQuantity = async (req, res) => {
  try {
    ...
    const updatedBook = await userCollection.findByIdAndUpdate(bookId, {
      quantity,
    })
    if (!updatedBook) {
